...
const readPkgUp = {}
readPkgUp.resolved = { path: Symbol('path'), packageJson: { existing: Symbol('existing') } }
...
const added = Symbol('packageJson.added')

const syncModifierFn = packageJson => {
  packageJson.added = added
  return packageJson
}
const asyncModifierFn = packageJson => {
  packageJson.added = added
...
test('`writePkg` call 2nd arg is `packageJson` of `readPkgUp`’s return value resolved', async t => {
  const subject = t.context.subject
  const writePkgSpy = t.context.writePkgSpy
  await subject(syncModifierFn)
  t.is(writePkgSpy.args[0][1], readPkgUp.resolved.packageJson)
})
